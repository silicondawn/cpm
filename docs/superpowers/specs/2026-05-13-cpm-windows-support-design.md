...
Fork lives at `github.com/silicondawn/cpm`; scoop bucket at `github.com/silicondawn/scoop-cpm`.
...
| Scoop bucket | **New** — `silicondawn/scoop-cpm` | Generated by goreleaser |
...
1. **GitHub fork owner**: `silicondawn`. Repo at `github.com/silicondawn/cpm`. Scoop bucket at `github.com/silicondawn/scoop-cpm`.
2. **Module path**: keep `module github.com/jakubkontra/cpm` unchanged. Reason: friction-free cherry-pick from upstream + future PR back to upstream. A Go module path does not need to match the GitHub repo location; this is a common and supported fork pattern. If we ever publish a distinct module identity, renaming is a one-shot mechanical change.
3. **Branch strategy**: `main` tracks upstream. `windows-support` is the development branch where all Windows work lands. After feature is stable, merge `windows-support` → `main` and tag a release.
